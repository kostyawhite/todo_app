...
func (r *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	getAllQuery := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul ON tl.id = ul.list_id WHERE ul.user_id = $1",
...
func (r *TodoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {
	...
	getByIdQuery := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul ON tl.id = ul.list_id
								WHERE ul.user_id = $1 AND ul.list_id = $2`,
		todoListsTable, usersListsTable)
	err := r.db.Get(&list, getByIdQuery, userId, listId)
...
func (r *TodoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
	...
	logrus.Debugf("updateQeury: %s", query)
